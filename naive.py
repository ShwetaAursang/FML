...
class Naive:
	...
	def classify(self):
		...
		pos2=positive[positive['Temperature']=='cool'].shape[0]
		...
		neg2=negative[negative['Temperature']=='cool'].shape[0]
	...
	def final(self):
		...
		map_pos=positive*(self.allpos[0])*(self.allpos[1])*(self.allpos[2])*(self.allpos[3])

		map_neg=negative*(self.allneg[0])*(self.allneg[1])*(self.allneg[2])*(self.allneg[3])
